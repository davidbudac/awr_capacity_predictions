...
-- views, then the three persisted tables. Idempotent: safe to run
...
-- The three persisted tables outlive 00_drop (which preserves them across a
-- re-install). A true uninstall removes them here.
BEGIN
    FOR t IN (SELECT 'CAP_ML_MODEL' AS n FROM dual
              UNION ALL SELECT 'CAP_TBSPC_OVERRIDE' FROM dual
